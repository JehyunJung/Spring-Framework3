...
public class ValidationItemControllerV2 {
   ...
   /* @PostMapping("/add")
    ...
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        if(item.getQuantity() == null || item.getQuantity() >=9999) {
            bindingResult.addError(new FieldError("item","quantity", item.getQuantity(),false,null,null, "수량은 최대 9999개 입니다."));
        }
        ...
        if(item.getPrice() != null && item.getQuantity() != null){
            ...
            log.info("price: {} * quantity: {} = {}", item.getPrice(), item.getQuantity(), resultPrice);
            if(resultPrice < 10000){
                bindingResult.addError(new ObjectError("item",null,null, "가격과 수량의 곱은 10000이상 이어야합니다. 현재값: " + resultPrice));
            }
        }
